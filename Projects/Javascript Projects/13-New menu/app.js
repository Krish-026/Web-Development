...
// get only unique categories - IMP
...
function displayMenuButtons2() {
  ...
  categories.forEach(function (value) {
    categoriesBtn += `<button class="filter-btn" type="button" 
    data-id="${value}">
    ${value}
    </button>`;
  });
  ...
  filtersBtns.forEach(function (btn) {
    btn.addEventListener("click", function (e) {
      const category = e.currentTarget.dataset.id;
      const menuCategory = menuByCategory[category] || menu;
      displayMenuItems(menuCategory);
    });
  });
...
function displayMenuButtons3() {
  // const categories = new Set(['all', ...menu.map(item => item.category)]);
  const categories = new Set(['all', ...menu.map(item => item.category)]);
  const categoriesBtn = Array.from(categories)
    .map(category => `<button class="filter-btn" type="button" data-id="${category}">
  ${category}
  </button>`)
    .join('');
